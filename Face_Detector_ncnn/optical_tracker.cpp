...
/* -------------------------CODE-----------------------------*/
double OpticalTracker::euclid_dist(const Point2f *point1, const Point2f *point2) {
    /*
     * 欧式距离
...
void OpticalTracker::pairwise_dist(const Point2f *features, double *edist, int npoin) {
    /*
...
void OpticalTracker::ncc_filter(cv::Mat frame1, cv::Mat frame2, Point2f *prev_feat, Point2f *curr_feat,
...
void OpticalTracker::fb_filter(const Point2f *prev_features, const Point2f *backward_features,
...
void OpticalTracker::bbox_move(const Point2f *prev_feat, const Point2f *curr_feat, const int npoin,
...
void OpticalTracker::init(cv::Mat &img) {
    //保存第一帧图像
...
void OpticalTracker::draw_point(cv::Mat &img, vector<Point2f> &points, float scale, cv::Scalar &color) {
    for (int i = 0; i < points.size(); i++) {
        cv::circle(img, cv::Point(points[i].x * scale, points[i].y * scale), 0.5, color, 4);
    }
}

cv::Rect OpticalTracker::track(cv::Mat &img, cv::Mat before, double x1, double y1, double x2, double y2) {
    timer.tic();
    bbox_x = x1 / Scale;
    bbox_y = y1 / Scale;
    ...
    if (DEBUG) {
        ...
        draw_point(img, frame1_features, Scale, color);
    }
    //金字塔图像
    ...
    calcOpticalFlowPyrLK(frame1_1C, frame2_1C, frame1_features, frame2_features, status, err, Size(7, 7), 3, termcrit,
                         0, 0.001);
    ...
    for (int i = 0; i < npoints; i++) {
        if ( /*fb_pass[i] &&ncc_pass[i]  &&*/  (frame2_features[i].x > bbox_x) && (frame2_features[i].y > bbox_y) &&
                                               (frame2_features[i].x < bbox_x + bbox_width) &&
                                               (frame2_features[i].y < bbox_y + bbox_height)) {
            //总的好点数
            pcount++;
        }
    }
    if (DEBUG) {
        ...
        cv::Scalar color(0, 125, 0);
        draw_point(img, frame2_features, Scale, color);
    }
#if 0
    if((median)<0.10 )
    {
