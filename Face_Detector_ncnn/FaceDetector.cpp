...
//    cv::Size size = cv::Size(bgr.cols*scale, bgr.rows*scale);
    cv::resize(bgr, img_scale, cv::Size(bgr.cols * scale, bgr.rows * scale));
    ncnn::Mat img = ncnn::Mat::from_pixels((unsigned char *) bgr.data, ncnn::Mat::PIXEL_BGR, bgr.cols, bgr.rows);
    ncnn::Mat in = ncnn::Mat::from_pixels_resize(bgr.data, ncnn::Mat::PIXEL_BGR, bgr.cols, bgr.rows, img_scale.cols,
                                                 img_scale.rows);
    ...
    for (int j = 0; j < total_box.size(); ++j) {
        total_box[j].x1 = total_box[j].x1 / scale;
        total_box[j].y1 = total_box[j].y1 / scale;
        total_box[j].x2 = total_box[j].x2 / scale;
        total_box[j].y2 = total_box[j].y2 / scale;
        for (int k = 0; k < 5; k++) {
            total_box[j].point[k]._x = total_box[j].point[k]._x / scale;
            total_box[j].point[k]._y = total_box[j].point[k]._y / scale;
        }
        boxes.push_back(total_box[j]);
    }
}

inline int area(bbox b) {
    return static_cast<int>((b.x2 - b.x1) * (b.y2 - b.y1));
}
